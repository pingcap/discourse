...
class UserMerger
  ...
  def merge_given_daily_likes
    sql = <<~SQL
      INSERT INTO given_daily_likes(user_id, likes_given, given_date, limit_reached)
        SELECT
        ...
        GROUP BY given_date
      ON DUPLICATE KEY
        UPDATE likes_given = likes_given, limit_reached = limit_reached
    SQL
  ...
  def merge_post_timings
    ...
    sql = <<~SQL
      ...
      INNER JOIN post_timings AS s ON t.user_id = :target_user_id AND s.user_id = :source_user_id
            AND t.topic_id = s.topic_id AND t.post_number = s.post_number
      SET t.msecs = t.msecs + s.msecs
    SQL
  ...
  def merge_user_visits
    ...
    sql = <<~SQL
      ...
      INNER JOIN user_visits AS s ON t.user_id = :target_user_id AND s.user_id = :source_user_id
            AND t.visited_at = s.visited_at
      SET t.posts_read = t.posts_read + s.posts_read,
        t.mobile       = t.mobile OR s.mobile,
        t.time_read    = t.time_read + s.time_read
    SQL
  ...
  def update_user_stats
    ...
    DB.exec(<<~SQL, target_user_id: @target_user.id)
      ...
      SET us.time_read  = COALESCE(x.time_read, 0),
        us.days_visited = COALESCE(x.days_visited, 0)
    SQL
    ...
    DB.exec(<<~SQL, source_user_id: @source_user.id, target_user_id: @target_user.id)
      ...
      INNER JOIN user_stats AS s
      ON t.user_id = :target_user_id AND s.user_id = :source_user_id
      SET t.likes_given         = t.likes_given + s.likes_given,
        t.likes_received        = t.likes_received + s.likes_received,
        t.new_since             = LEAST(t.new_since, s.new_since),
        t.read_faq              = LEAST(t.read_faq, s.read_faq),
        t.first_post_created_at = LEAST(t.first_post_created_at, s.first_post_created_at)
    SQL
  end
  ...
  def merge_user_attributes
    DB.exec(<<~SQL, source_user_id: @source_user.id, target_user_id: @target_user.id)
      UPDATE users AS t
      INNER JOIN users AS s ON t.id = :target_user_id AND s.id = :source_user_id
      SET t.created_at              = LEAST(t.created_at, s.created_at),
        t.updated_at                = LEAST(t.updated_at, s.updated_at),
        t.seen_notification_id      = GREATEST(t.seen_notification_id, s.seen_notification_id),
        t.last_posted_at            = GREATEST(t.last_seen_at, s.last_seen_at),
        t.last_seen_at              = GREATEST(t.last_seen_at, s.last_seen_at),
        t.admin                     = t.admin OR s.admin,
        t.last_emailed_at           = GREATEST(t.last_emailed_at, s.last_emailed_at),
        t.trust_level               = GREATEST(t.trust_level, s.trust_level),
        t.previous_visit_at         = GREATEST(t.previous_visit_at, s.previous_visit_at),
        t.date_of_birth             = COALESCE(t.date_of_birth, s.date_of_birth),
        t.ip_address                = COALESCE(t.ip_address, s.ip_address),
        t.moderator                 = t.moderator OR s.moderator,
        t.title                     = COALESCE(t.title, s.title),
        t.primary_group_id          = COALESCE(t.primary_group_id, s.primary_group_id),
        t.registration_ip_address   = COALESCE(t.registration_ip_address, s.registration_ip_address),
        t.first_seen_at             = LEAST(t.first_seen_at, s.first_seen_at),
        t.manual_locked_trust_level = GREATEST(t.manual_locked_trust_level, s.manual_locked_trust_level)
    SQL

    DB.exec(<<~SQL, source_user_id: @source_user.id, target_user_id: @target_user.id)
      UPDATE user_profiles AS t
      INNER JOIN user_profiles AS s ON t.user_id = :target_user_id AND s.user_id = :source_user_id
      SET t.location           = COALESCE(t.location, s.location),
        t.website              = COALESCE(t.website, s.website),
        t.bio_raw              = COALESCE(t.bio_raw, s.bio_raw),
        t.bio_cooked           = COALESCE(t.bio_cooked, s.bio_cooked),
        t.bio_cooked_version   = COALESCE(t.bio_cooked_version, s.bio_cooked_version),
        t.profile_background_upload_id = COALESCE(t.profile_background_upload_id, s.profile_background_upload_id),
        t.dismissed_banner_key = COALESCE(t.dismissed_banner_key, s.dismissed_banner_key),
        t.badge_granted_title  = t.badge_granted_title OR s.badge_granted_title,
        t.card_background_upload_id = COALESCE(t.card_background_upload_id, s.card_background_upload_id),
        t.views                = t.views + s.views
    SQL
  end
  ...
  def update_user_ids
    ...
    update_user_id(:user_actions,
                   ...
                   conditions: ["x.action_type = y.action_type",
                                "x.target_topic_id <=> y.target_topic_id",
                                "x.target_post_id <=> y.target_post_id",
                                "(x.acting_user_id IN (:source_user_id, :target_user_id) OR x.acting_user_id <=> y.acting_user_id)"])
    update_user_id(:user_actions,
    ...
    update_user_id(:user_emails, conditions: "x.email = y.email OR y.primary = false", updates: 'x.`primary` = false')
  ...
  def update_user_id_sql_builder(table_name, opts = {})
    ...
    builder = DB.build(<<~SQL)
      ...
      WHERE x.#{user_id_column_name} = :source_user_id AND NOT EXISTS(
          SELECT 1
          FROM (select * from #{table_name}) AS y
          /*where*/
      )
    SQL

    builder.set("x.`#{user_id_column_name}` = :target_user_id")
