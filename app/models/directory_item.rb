...
class DirectoryItem < ActiveRecord::Base
  ...
  def self.refresh_period!(period_type, force: false)
    ...
    ActiveRecord::Base.transaction do
      ...
      DB.exec "DELETE directory_items
                 FROM directory_items
                LEFT JOIN users u ON (u.id = user_id AND u.active AND u.silenced_till IS NULL AND u.id > 0)
                WHERE
                      u.id IS NULL AND
                      directory_items.period_type = :period_type", period_type: period_types[period_type]
      ...
      if period_type == :all
        DB.exec <<~SQL
          UPDATE user_stats s
          INNER JOIN directory_items d ON s.user_id = d.user_id
          SET s.likes_given         = d.likes_given,
              s.likes_received      = d.likes_received,
              s.topic_count         = d.topic_count,
              s.post_count          = d.post_count
          WHERE d.period_type = 1 AND
