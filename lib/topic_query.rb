...
class TopicQuery
  ...
  def apply_ordering(result, options)
    ...
    if sort_column.start_with?('custom_fields')
      ...
      return result.order("(SELECT CASE WHEN EXISTS (SELECT true FROM topic_custom_fields tcf WHERE CAST(cf.topic_id AS UNSIGNED) = CAST(topics.id AS UNSIGNED) AND tcf.name = '#{field}') THEN (SELECT CAST(value AS UNSIGNED) FROM topic_custom_fields tcf WHERE CAST(tcf.topic_id AS UNSIGNED) = CAST(topics.id AS UNSIGNED) AND tcf.name = '#{field}') ELSE 0 END) #{sort_dir}")
    end
